package com.learn.couchbase.controllers;

import com.learn.couchbase.data.Blacklist;
import com.learn.couchbase.services.BlacklistService;
...
public class BlacklistController {
	...
	public ResponseEntity<Set<String>> getShopBlacklist(@PathVariable("shopName") String shopName) {
		...
		try {
			...
			result = new ResponseEntity<Set<String>>(blacklist, HttpStatus.OK);
		}catch(Exception e) {
			LOG.error(e.getMessage());
			result = new ResponseEntity<Set<String>>(new LinkedHashSet<>(), HttpStatus.INTERNAL_SERVER_ERROR);
		}
